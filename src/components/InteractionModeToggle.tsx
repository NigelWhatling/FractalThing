...
const InteractionModeToggle = ({ value, onChange, onReset }: InteractionModeToggleProps) => {
  const baseClasses =
    'flex h-9 w-9 touch-manipulation items-center justify-center rounded-full border border-white/60 bg-white/70 text-sm text-slate-700 shadow-[0_6px_18px_rgba(15,23,42,0.12)] backdrop-blur-md transition hover:border-white/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/50 motion-reduce:transition-none dark:border-white/10 dark:bg-slate-900/70 dark:text-slate-200 dark:shadow-[0_8px_22px_rgba(0,0,0,0.5)] dark:hover:border-white/30';
  const activeClasses =
  ...
  return (
    <div
      className="fixed right-3 top-3 z-50 flex gap-2 rounded-full border border-white/60 bg-white/60 p-1 shadow-[0_10px_24px_rgba(15,23,42,0.15)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/60 dark:shadow-[0_12px_30px_rgba(0,0,0,0.55)]"
      role="group"
